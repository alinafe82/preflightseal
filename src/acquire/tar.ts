...
export async function extractGitHubTarBuffer(
  ...
  for (const entry of entries) {
    ...
    if (entry.type === "directory") {
      await mkdir(absolute, { recursive: true, mode: entry.mode || 0o755 });
      directories += 1;
      continue;
    }
...
export function parseTar(tar: Buffer, limits = defaultArchiveLimits): ParsedTarEntry[] {
  ...
  while (offset + 512 <= tar.length) {
    ...
    if (size > limits.maxFileBytes) {
      throw new Error(`archive entry exceeds maximum file size: ${relativePath}`);
    }
    totalBytes += size;
    ...
    if (entries.length + 1 > limits.maxFiles) {
      throw new Error("archive exceeds maximum file count");
    }
    entries.push({
      relativePath,
      type: "file",
